...
public class ShowAllAreasValuesRaw_Patched extends ShowAllAreasValues 
{
    public static void main(String[] args)
    {
        Util.out("Сырые данные изданий ЦСК и УГВИ без коррекции (кроме опечаток выявленных по перекрёстным суммам и патчей), ");
        ...
        try
        {
            ...
            rawShowAllAreasValuesPatched(exportData).show_values_all();
            exportData.export("c:\\@\\pre1917\\CSK-UGVI-raw-patched.csv");
        }
        catch (Throwable ex)
        {
            Util.err("** Exception: ");
    ...
    private ShowAllAreasValuesRaw_Patched() throws Exception
    {
    }
}
